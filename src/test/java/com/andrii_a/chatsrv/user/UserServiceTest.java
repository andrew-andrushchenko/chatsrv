...
class UserServiceTest {
    ...
    public void should_successfully_get_online_users() {
        ...
        Mockito.when(userRepository.findAllByStatus(UserStatus.ONLINE)).thenReturn(arrayList);

        // Then
        var listOfConnectedUsers = userService.findConnectedUsers();
        assertEquals(listOfConnectedUsers.size(), 2);
        listOfConnectedUsers.forEach(user -> assertEquals(user.getStatus(), UserStatus.ONLINE));
    }
}